...
"""
def get_file_groups_to_process() -> tuple[list[FileGroup], list[FileGroup]]:
    ...
    copy_file_groups = [FileGroup(raw_files) for raw_files in config.FILES_TO_PROCESS if raw_files['type'] == 'copy']
    markdown_file_groups = [FileGroup(raw_files) for raw_files in config.FILES_TO_PROCESS if raw_files['type'] == 'text']
